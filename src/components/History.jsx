...
export default function History({ transactions, onDelete }) {
  ...
  return (
    <section className="card animate-enter p-5 sm:p-6" aria-labelledby="history-heading">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.16em] text-brand-600">Records</p>
          <h2 id="history-heading" className="mt-1 text-xl font-extrabold tracking-tight">Transactions</h2>
        </div>
      ...
      {shown.length === 0 ? (
        <div className="grid min-h-56 place-items-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 p-6 text-center">
          <div>
        ...
        <div className="space-y-2.5">
          {shown.map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} onDelete={onDelete} showDate={view === 'all'} />)}
        </div>
      )}
    </section>
  )
...
function TransactionRow({ transaction, onDelete, showDate }) {
  ...
  return (
    <article className="group rounded-2xl border border-slate-100 bg-slate-50/70 p-3.5 transition hover:border-slate-200 hover:bg-white">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="mt-0.5 text-[11px] font-medium text-slate-400">{showDate ? `${formatDate(transaction.createdAt)} · ` : ''}{formatTime(transaction.createdAt)} · 100K = {formatRate(transaction.rate)} THB</p>
            </div>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 border-t border-slate-200/70 pt-3">
            ...
            <Value label="Profit" value={formatNumber(transaction.profit)} accent={transaction.profit >= 0} negative={transaction.profit < 0} />
          </div>
        </div>
      </div>
    </article>
...
function Value({ label, value, accent, negative }) {
  return <div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">{label}</p><p className={`tabular mt-0.5 truncate text-xs font-extrabold sm:text-sm ${accent ? 'text-emerald-600' : negative ? 'text-rose-600' : 'text-slate-700'}`}>{value}</p></div>
}
